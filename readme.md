...
Укажите на вашей странице имя локали, укажите путь к таблицам перевода, выберите имя текстового домена, например:
...
Сделайте перевов фраз на ваш язык. Сохраните результат. Будет автоматически создат файл машинного перевода `calendar.mo`, скопируйте оба файла в соответствующий каталог на сервере.

Наиболее трудомкий процесс - это перевод json-файлов. В сети есть большое количество как текстовых редакторов, так и специализированных редакторв json-файлов.
Подлежат переводу поля с именами только `"title":` и `"text":`. Поля с другими именами (например, `"gender":`, `"type":`) перводить **НЕЛЬЗЯ!**
